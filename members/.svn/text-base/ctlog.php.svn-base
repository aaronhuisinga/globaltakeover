...
if ($id != "" && isset($_COOKIE["id"])) {
	$query = "SELECT health, username, corps FROM Players WHERE id='$id'";
...
<center>
<div id="usertable">
<h1>Recent Token Transactions</h1>
<table width=50%><tr>
...
<?
	$gather=mysql_query("SELECT * FROM clogs WHERE corp='$c' AND type='token' AND outin='out' ORDER BY date DESC LIMIT 50;"); 
...
<?
$gather=mysql_query("SELECT * FROM clogs WHERE corp='$c' AND type='token' AND outin='in' ORDER BY date DESC LIMIT 50;"); 
	while ($row=mysql_fetch_array($gather)){
	$id=stripslashes($row['id']);
