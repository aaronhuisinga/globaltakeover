	...
	if ($l == $orl) {
		if ($u == $idri) {
			...
			echo "<head><meta HTTP-EQUIV=\"REFRESH\" content=\"2; url=OR.php\"></head>
				  <div align=\"center\" id=\"crimestext\">You have joined the Organized Robbery as the Driver. Redirecting...</div>";
			include("footer.php");
			exit();
		} elseif ($u == $iee) {
			mysql_query("UPDATE Robbery SET ee='$u', inviteee='None' WHERE id='$id' LIMIT 1;");
			echo "<head><meta HTTP-EQUIV=\"REFRESH\" content=\"2; url=OR.php\"></head>
				  <div align=\"center\" id=\"crimestext\">You have joined the Organized Robbery as the Artillery. Redirecting...</div>";
			include("footer.php");
			exit();
		} elseif ($u == $iwep) {
...
if ($_REQUEST['submit']) {
	...
	$tp = $dp + $wp + $ep;
	if ($tp > 100) {
	echo "<div id=\"crimestext\" align=\"center\">You cannot pay out over 100% of the shares!<br /><a href=\"OR.php\">Go back.</a></div>";
	include("footer.php");
	exit();
	}
	$dri=$_POST['d'];
	...
	if ($duplicates==1) {
		echo "<div id=\"crimestext\" align=\"center\">You cannot invite the same person multiple times.<br /><a href=\"OR.php\">Go back.</a></div>";
		include("footer.php");
		exit();
	}
	if ($u == $lead) {
	...
	foreach ($arr as $pos) {
		...
		if ($username != $idri AND $username != $iee AND $username != $iwep) {
			if (mysql_num_rows($sql) != 1) {
				echo "<div id=\"crimestext\" align=\"center\">$pos is not a valid username!<br /><a href=\"OR.php\">Go back.</a></div>";
				include("footer.php");
				exit();
			}else{
				if ($yourip == $lastip OR $yourip == $regip) {
					echo "<div id=\"crimestext\" align=\"center\">You cannot invite a player that has used the same IP as you!<br /><a href=\"OR.php\">Go back.</a></div>";
					include("footer.php");
					exit();
				}
				if ($dead == 1 OR $banned == 1) {
					echo "<div id=\"crimestext\" align=\"center\">$username is either dead or banned!<br /><a href=\"OR.php\">Go back.</a></div>";
					include("footer.php");
					exit();
				} else {
					$sql=mysql_query("SELECT * FROM Robbery WHERE leader='$username' OR driver='$username' OR ee='$username' OR wep='$username' LIMIT 1;");
					if (mysql_num_rows($sql) != 0) {
						echo "<div id=\"crimestext\" align=\"center\">$username is already in an Organized Robbery!<br /><a href=\"OR.php\">Go back.</a></div>";
						include("footer.php");
						exit();
					} else {
						mysql_query("SELECT * FROM Robbery WHERE invited='$username' OR invitew='$username' OR inviteee='$username' LIMIT 1;");
						if (mysql_num_rows($sql) != 0) {
							echo "<div id=\"crimestext\" align=\"center\">$username has already been invited to another Organized Robbery.<br /><a href=\"OR.php\">Go back.</a></div>";
							include("footer.php");
							exit();
						} else {
							...
							$ORsd = $ortime - $current;
							if ($ORsd > 0) {
								echo "<div id=\"crimestext\" align=\"center\">$username has already done a Robbery in the last 8 hours!<br /><a href=\"OR.php\">Go back.</a></div>";
								include("footer.php");
								exit();
							} else {
							if ($pos == $dri) { $posi='Driver'; $percent=$dp; $dri=$username; } elseif ($pos == $eee) { $posi='Artillery'; $percent=$ep; $eee=$username; } elseif ($pos == $weep) { $posi='Weapons Expert'; $percent=$wp; $weep=$username; }
							$subject = htmlspecialchars(addslashes("Organized Robbery Invitation"));
		...
		echo "<div id=\"crimestext\" align=\"center\">You cannot invite people to the Organized Robbery, because you are not the Hacker.<br /><a href=\"OR.php\">Go back.</a></div>";
		include("footer.php");
		exit();
	}
} else {
...
if ($cid == NULL) {
	if ($money < 300000) {
	echo "<div id=\"crimestext\"><center>You don't have enough money to start an Organized Robbery!<br /><a href=\"OR.php\">Go back.</a></div></center>";
	include("footer.php");
	exit();
	} elseif ($money >= 300000) {
			$nm=$money-300000;
	...
	$sql = mysql_query("SELECT * FROM Robbery WHERE leader='$u' OR driver='$u' OR ee='$u' OR wep='$u'");
	$row = mysql_fetch_array($sql);
	$cid = $row['id'];
	if ($cid == NULL) {
		$sqlq = mysql_query("SELECT * FROM Robbery WHERE invited='$u' OR invitew='$u' OR inviteee='$u'");
		if (mysql_num_rows($sqlq) == 0) {
			echo "<div id=\"crimestext\"><center><h2>Organized Robbery</h2><br /><br />
			You are currently not in an Organized Robbery. To start one, it costs $300,000<br />";
			echo '<form action="SOR.php" method="POST"><input type="submit" name="SOR" Value="Attack By Sea!">
			...
			$irow = mysql_fetch_array(mysql_query("SELECT id FROM Players WHERE username='$lead'"));
			$lid = $irow[0];
			if ($idri == $u) {
				...
				Location: $loc<br />
				Percentage: $dp%<br />
				<form action=\"OR.php\" method=\"POST\"><input type=\"submit\" Value=\"Accept\" Name=\"Yes\"> or <input type=\"submit\" Value=\"Decline\" Name=\"No\">
				</form></div>";
				include("footer.php");
				exit();
			} elseif ($iwep == $u) {
		...
		echo "<form action=\"ORdo.php\" method=\"POST\"><input name=\"Commit\" type=\"submit\" value=\"Commit\"></form>";
		} elseif ($lead == $u) {
		...
		<tr class=\"top\"><td align=\"center\">Position</td> <td align=\"center\">Structure</td> <td align=\"center\">Weapons</td> <td align=\"center\">Vehicle</td> <td align=\"center\">Artillery</td> <td align=\"center\">Hacking</td> <tr>
		...
		<tr><td class=\"top\" align=\"center\">Driver</td> <td align=\"center\">";if($did != NULL) {echo"<a href=\"profile.php?id=$did\">$driver</a>";} else { echo"$driver"; } echo"</td> <td align=\"center\">"; if ($dgun == NULL) { echo "---"; } else { echo "$dgun"; } echo "</td></tr>
		<tr><td class=\"top\" align=\"center\">Artillery</td> <td align=\"center\">";if($eid != NULL) {echo"<a href=\"profile.php?id=$eid\">$ee</a>";} else { echo"$ee"; } echo"</td> <td align=\"center\">"; if ($egun == NULL) { echo "---"; } else { echo "$egun"; } echo "</td></tr>
		...
		if ($u == $lead) {
		echo "<form action=\"OR.php\" method=\"POST\"><br /><h1 align=\"center\">Invitations</h1><table>";
		if ($driver == 'None') {	
			if ($idriver == 'None') {		
				echo "<tr><td>Driver:</td> <td><input type=\"text\" name=\"d\"></td><td>Share: <select name='Dpercent'> 
76%<option value='77'>77%<option value='78'>78%<option value='79'>79%<option value='80'>80%<option value='81'>81%<option value='82'>82%<option value='83'>83%<option value='84'>84%<option value='85'>85%<option value='86'>86%<option value='87'>87%<option value='88'>88%<option value='89'>89%<option value='90'>90%<option value='91'>91%<option value='92'>92%<option value='93'>93%<option value='94'>94%<option value='95'>95%<option value='96'>96%<option value='97'>97%<option value='98'>98%<option value='99'>99%<option value='100'>100%
		...
		echo "<br />Welcome $u<br /> You are the current Driver!";
		?><form action="ORdriver.php" method="POST">
		<table id="usertable" width="40%">
		<tr class="top">
		...
		<?
		$query=mysql_query("SELECT username FROM Players WHERE id='{$_COOKIE['id']}' LIMIT 1;"); 
		$row=mysql_fetch_array($query);
		$u = $row[0];
		$query=mysql_query("SELECT * FROM garage WHERE username='$u' AND percent=100 ORDER BY car ASC"); 
		while ($row=mysql_fetch_array($query)){
		$car=$row['car'];
		$caid = $row['id'];
		?>		
  		<tr><td width="10%"><? echo "$car"; ?></td><td width="1%"><input name="radio" type="radio" value="<? echo "$caid"; ?>"></td></tr>
		<?
		}
		?>
		</table>
		...
		echo "<br />Welcome $u<br /> You are in charge of Artillery!";
		?><form action="OReee.php" method="POST">
		<table id="usertable" width="40%">
		<tr class="top"><td width="10%"><b>Artillery</b></td><td width="1%"><b>Select</b></td></tr>
   		<tr><td>RH-202 Anti-Aircraft Gun ($250,000)</td><td><input name="radio" type="radio" value="RH-202 Anti-Aircraft Gun"></td></tr>
   		<tr><td>MLRS M270 12x227mm Multiple Rocket Launcher ($200,000)</td><td><input name="radio" type="radio" value="MLRS M270 12x227mm Multiple Rocket Launcher"></td></tr>
   		<tr><td>M53/59 Self-Propelled Twin 30mm Cannon ($150,000)</td><td><input name="radio" type="radio" value="M53/59 Self-Propelled Twin 30mm Cannon"></td></tr>
   		<tr><td>PLZ-45 Self-Propelled 155mm Howitzer ($100,000)</td><td><input name="radio" type="radio" value="PLZ-45 Self-Propelled 155mm Howitzer"></td></tr>
   		</table>
   		<input name="update" type="submit" value="Update!">
		</form>
		<?
		...
		echo ("<table>");
		echo "<br />Welcome $u<br /> You are the current Weapons Expert!";
		...
		echo '<form action="ORweep.php" method="POST"><tr><td align="right">Leader:</td> <td><select name="lselect">';
		echo "<option value=\"none\">No Change</option>";
 		foreach ($weapons as $msg) {
 		echo "$msg";
 		}
 		echo '</select></td></tr>';
 		echo '<tr><td align="right">Driver:</td> <td><select name="dselect">';
 		echo "<option value=\"none\">No Change</option>";
 		foreach ($weapons as $msg) {
 		echo "$msg";
 		}
 		echo '</select></td></tr>';
 		echo '<tr><td align="right">Artillery:</td> <td><select name="eeselect">';
 		echo "<option value=\"none\">No Change</option>";
 		foreach ($weapons as $msg) {
 		echo "$msg";
 		}
 		echo '</select></td></tr>';
 		echo '<tr><td align="right">Weapons Expert:</td> <td><select name="weselect">';
 		echo "<option value=\"none\">No Change</option>";
 		foreach ($weapons as $msg) {
 		echo "$msg";
 		}
