...
?>
<br /><br />
<form action="csellh.php" method="post"><div align="center" id="garage">
<h1 align="center"><? echo("$cname"); ?> Hanger</h1>
<table>
  <tr class="top">
    <td><b>Plane</b></td>
...
<?	 
$query=mysql_query("SELECT * FROM changer WHERE corp='$corp' ORDER BY plane ASC"); 
...
<table width="50%" align="center">
	<tr><td align="center" colspan="2"><b>Your Hanger</b><br /></td></tr>
	</div>
  	<form action="cdonateplane.php" method="POST">
		<div align="center">
		...
		<?
		$query=mysql_query("SELECT username FROM Players WHERE id='{$_COOKIE['id']}'"); 
		$row=mysql_fetch_array($query);

		$u = $row[0];
	 
		$query=mysql_query("SELECT id, plane, percent FROM hanger WHERE username='$u'"); 
		while ($row=mysql_fetch_array($query)){
		$ucar=$row['plane'];
		$ucaid = $row['id'];
...
<h1 align="center"><? echo("$corp"); ?> Hanger</h1>
<table>
  <tr class="top">
    <td><b>Plane</b></td>
    <td width="11%"><b>Percent</b></td>
...
echo ("There has been an error. Message an Admin immediately. (Code= changer vs status)");
}
}
?>