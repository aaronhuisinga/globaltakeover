...
exit();
	...
	$thealth = 100;
	$tarmor = 0;
	$trank = 'Contract Killer';
	$tid = $row['id'];
	$tstatus = $row['dead'];
	
	$sql = mysql_query("SELECT `Leon`, bulletused FROM `mission6` WHERE username = '$user'");
	...
	<a href=\"shootL.php?id={$id}\">Go back.</a></center></div>");
	exit();
	...
	*/
	...
	mysql_query("UPDATE mission6 SET bulletused = '$new_bullet' WHERE username = '$user'");
	$sql = mysql_query("UPDATE mission6 SET missionstats = '3' WHERE username = '$user'");
	// Drop Props
	...
	<form action=\"mission6.php\" method= \"POST\"><input type=\"submit\" name=\"Finish\" value=\"Finish Mission!\"></form></center></div>";
	...
	$date = (date("M d Y h:i:s A"));					
	mysql_query("INSERT INTO Playerbullets (amount, date, username, outcome, btime, used) VALUES ('$kill_bullets', '$date', '$user', 'Loss', '$current', 'Shooting')");			
	echo "<div id=\"gameplay\"><center>You ambushed <a href=\"profile.php?id={$tid}\">$target</a>, but he was ready for you and fled!<br />
	...
	mysql_query("DELETE FROM mspies WHERE target='$target' and hunter='$user'");
	mysql_query("UPDATE mission6 SET bulletused = '$new_bullet' WHERE username = '$user'");
	$sql = mysql_query("UPDATE mission6 SET missionstats = '1' WHERE username = '$user'");
	...
	echo("<div id=\"gameplay\"><center><form name='shoot' action='shootL.php?id={$id}' method='POST'>
	<table id=\"usertable\">
