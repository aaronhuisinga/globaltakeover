...
$save = rand(1, 100);
if ($location != $di) {
echo '<div id="crimestext" align="center">You are not in the same location as Dmitri!</div>';
exit();
}
if ($D != 0) {
echo '<div id="crimestext" align="center">You have already saved Dmitri!</div>';
exit();
}
$save = rand (1, 100);
if ($save >= 1 AND $save <= 85) {
		...
		if ($rank == 'Wannabe') {
			...
			echo "<div id=\"crimestext\"><center>You failed to save Dmitri! Hope you like being locked up!<br /><a href=\"prison.php\">Go back.</center></div>";
		...
		mysql_query("UPDATE mission2 SET Dmitri = 1 WHERE username = '$username' LIMIT 1;");
		if ($Ni == 0 OR $Na == 0) {
			echo "<div id=\"crimestext\"><center>You saved Dmitri! There are still members missing!<br /><a href=\"prison.php\">Go back.</center></div>";
			} else {
			echo '<div id="crimestext"><center>You saved Dmitri! You succesfully saved everyone!<br />
