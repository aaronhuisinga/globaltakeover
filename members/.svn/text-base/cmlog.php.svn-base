...
<center>
<div id="usertable">
<h1>Recent Money Transactions</h1>
<table width=50%><tr>
...
<?
	$gather=mysql_query("SELECT * FROM clogs WHERE corp='$c' AND type='money' AND outin='out' ORDER BY date DESC LIMIT 50;"); 
...
<?
$gather=mysql_query("SELECT * FROM clogs WHERE corp='$c' AND type='money' AND outin='in' ORDER BY date DESC LIMIT 50;"); 
	while ($row=mysql_fetch_array($gather)){
	$id=stripslashes($row['id']);
