...
$save = rand(1, 100);
if ($location != $nal) {
echo '<div id="crimestext" align="center">You are not in the same location as Natalya!</div>';
exit();
}
if ($Na != 0) {
echo '<div id="crimestext" align="center">You have already saved Natalya!</div>';
exit();
}
$save = rand (1, 100);
if ($save >= 1 AND $save <= 85) {
		...
		if ($rank == 'Wannabe') {
			...
			echo "<div id=\"crimestext\"><center>You failed to save Natalya! Hope you like being locked up!<br /><a href=\"prison.php\">Go back.</center></div>";
		...
		mysql_query("UPDATE mission2 SET Natalya = 1 WHERE username = '$username' LIMIT 1;");
		if ($D == 0 OR $Ni == 0) {
			echo "<div id=\"crimestext\"><center>You saved Natalya! There are still members missing!<br /><a href=\"prison.php\">Go back.</center></div>";
			} else {
			echo '<div id="crimestext"><center>You saved Natalya! You succesfully saved everyone!<br />
