...
exit();
	...
	$sql = mysql_query("SELECT `Sandrine`, bulletused FROM `mission9` WHERE username = '$user'");
	...
	<a href=\"shootS.php?id={$id}\">Go back.</a></center></div>");
	exit();
	...
	*/
	...
	mysql_query("UPDATE mission9 SET bulletused = '$new_bullet' WHERE username = '$user'");
	$sql = mysql_query("UPDATE mission9 SET missionstats = '3' WHERE username = '$user'");
	// Drop Props
	...
	<form action=\"mission9.php\" method= \"POST\"><input type=\"submit\" name=\"Finish\" value=\"Finish Mission!\"></form></center></div>";
	...
	mysql_query("DELETE FROM mspies WHERE target='$target' and hunter='$user'");
	mysql_query("UPDATE mission9 SET bulletused = '$new_bullet' WHERE username = '$user'");
	$sql = mysql_query("UPDATE mission9 SET missionstats = '1' WHERE username = '$user'");
	...
	echo("<div id=\"gameplay\"><center><form name='shoot' action='shootS.php?id={$id}' method='POST'>
	<table id=\"usertable\">
