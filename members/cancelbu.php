...
$row=mysql_fetch_array(mysql_query("SELECT username, bullets FROM Players WHERE id='{$_COOKIE['id']}' LIMIT 1;"));
$u = $row[0];
$t = $row[1];

$id = $_GET['id'];		
			$sql = mysql_query("SELECT * FROM bescrow WHERE id ='$id'");
			...
			if ($finish == 'Pending') {
			$nt = $t + $amount;
			mysql_query("UPDATE Players SET bullets = '$nt' WHERE id='{$_COOKIE['id']}' LIMIT 1;");
			...
			$message = htmlspecialchars(addslashes("$u has canceled the escrow for $amount Bullets"));
