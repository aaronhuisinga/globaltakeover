...
echo '<div id="crimestext" align="center">You need to be the Weapons Expert in an Organized Robbery first!<br /><a href="SOR.php">Go back.</a></div>';
include("footer.php");
exit();
} else {
if ($_REQUEST['update']) {
	...
	$newlwep = $amlweap - 1;
	if ($newlwep < 0) {
		echo '<div id="crimestext" align="center">You need to own the weapons before using them!<br /><a href="SOR.php">Go back.</a></div>';
		include("footer.php");
		exit();
	}
	...
	$query = mysql_query("UPDATE Robbery SET wwep = '$wweap' WHERE wep = '$u'");
		...
		echo "<head><meta HTTP-EQUIV=\"REFRESH\" content=\"2; url=DSOR.php\"></head>
			  <div align=\"center\" id=\"crimestext\">Successfully updated the weapons arrangement. Redirecting...</div>";
		include("footer.php");
} else {
echo '<div id="crimestext" align="center">Please go back and select weapons to use in the Organized Robbery!<br /><a href="SOR.php">Go back.</a></div>';
include("footer.php");
exit();
}
}
